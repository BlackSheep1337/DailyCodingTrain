...
// const userId = users.filter(({ firstName, lastName }) => firstName === 'Madison' && lastName === 'Marshall')
//   .map(({ id }) => id);
// console.log(userId)

// const whoWrot = users.filter(({ id }) => id === comments[0].userId)
//   .map(({ firstName, lastName }) => `${firstName} ${lastName}`);
// console.log(whoWrot);

// const whoWrot2 = users.filter(({ id }) => id === comments[3].userId)
//   .map(({ firstName, lastName }) => `${firstName} ${lastName}`)
// console.log(whoWrot2);

// // const usersWhoRote = users.map((comment) => {
// //   const { firstName, lastName } = users.find((user) => user.id === comment.userId)

// //   return {...comment, firstName, lastName }; 
// // });
// // console.log(usersWhoRote);

// const whoNotCommented = users.filter(({ id }) => !comments.find(({ userId }) => userId === id));
// console.log(whoNotCommented);


// const lengestWord = (word) => {
//   return Math.max(...word.split(' ').map((x) => x.length));
// // }
